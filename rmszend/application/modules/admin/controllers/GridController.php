...
class Admin_GridController extends Rhema_Controller_Abstract{
	...
	public function tableAction(){
		$this->_table = $this->_request->getParam('table', 'admin_element');
	...
	public function licenceAction(){
		...
		$modService = new Rhema_Model_Service();
		
		switch($task){
			case 'save' :  
				if($ssid){						
					$return = $modService->factory('admin_subsite_licence')->saveSiteLicence($params, $ssid); 
					$this->_sendAjaxMessage($return['message'], 'Update Licence', $return['type']);
				}
				break;				 
			default :		
				$licObject = $modService->factory('admin_licence');		
				$siteLics  = $licObject->getSiteLicences($ssid);
				$this->view->selected = Rhema_Util::generateOptionArray($siteLics, 'id', 'id');
				
				$allLics = $licObject->findAll();
				...
				case 'save' :
					{
						...
						$this->_sendAjaxMessage($done['message'], 'Update Template Sections', $done['type']);
