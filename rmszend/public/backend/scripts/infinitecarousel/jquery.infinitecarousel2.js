		...
		var options = $.extend(defaults, options);
			...
			if(o.displayThumbnails)
			{
				...
				for(i=0;i<=numImages-1;i++)
				{
					...
					if(i<=o.inView) $('#thumb'+randID+'_'+i).css({'border-color':activeColour});
					unviewable.push(i+1);
				}
			...
			function postMove()
			{
				...
				if(o.displayThumbnails) for(i=0;i<viewable.length;i++) $('#thumb'+randID+'_'+viewable[i]).css({'border-color':activeColour, 'border-width': '4px'}).animate({'opacity': 1},500);
