<?php
	...
	class Rhema_View_Helper_DisplayContent extends Zend_View_Helper_Abstract {
		...
		public function getItem($id, $typeId, $layoutId){
			...
			//$item['edit']			= isset($item['edit']) ? $item['edit'] : false ;

			$html[$type]      = $this->_memManager->create($content) ;
			$return['html']   = $html[$type]->value ;
			$return['data']   = $item ;
			$return['itemId'] = $id;
		...
		/*public function getLoginForm(){
			...
			$action         = $this->view->url($url,FRONT_MENU_ROUTE);
		...
		public function displayContent($sections, $layout, $divId = ''){
            ...
            $final = $this->_memManager->create($content) ;
		...
		public function replacePlaceholders($content){
			if(isset($this->siteDetails['AddressBook']['id'])){
				$address = Rhema_Util_String::addressArrayToString($this->siteDetails['AddressBook']);
			}else{
				$address = '';
			}
			$data = array(
				'[site-name]'         => $this->siteDetails['title'],
				'[site-email]'        => $this->siteDetails['contact_email'],
