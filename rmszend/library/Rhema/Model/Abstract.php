		...
		if($curTable){
			...
			if('del' == $operation){
				...
				if($request->getParam(Admin_Model_AdminMenu::TYPE_ROOTS_ONLY)){
					...
					$firstChild = Admin_Model_AdminMenu::getDefaultRow(array(),$model);
					$firstChild->save();
					$firstChild->getNode()->insertAsNextSiblingOf($row);
...
/*
	...
	public static function getDefaultTableDetails($model, $domain = null){
		...
		try{
			...
			$resp = $client->getDefaultTableDetails($model)->get();
	...
	public static function rmsTruncateTable($table){
		...
		return $doctrine->query("TRUNCATE TABLE $table");
	}
	 ...
	 */
	public static function getRemoveData($functionName, $args){
		$cacheLifetime = ($functionName == 'validate') ? 43200 : 10080 ; // 1 week
		$cacheId       = $functionName . '_' . md5(json_encode($args));		
