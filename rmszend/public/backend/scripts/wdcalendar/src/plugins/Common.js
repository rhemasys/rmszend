			...
			function PopUpCenterWindow(URLStr, width, height, newWin,
					scrollbars){
				var popUpWin = 0;
				...
				if(newWin){
					open(URLStr, '',
							'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars='
									+ scrollbars
									+ ',resizable=yes,copyhistory=yes,width='
			...
			function OpenModelWindow(url, option){
				...
				try{
					if(parent != null && parent.$ != null
							&& parent.$.ShowIfrmDailog != undefined){
				...
				fun(url, option);
				// jQuery.ShowIfrmDailog(url, option);
			}
			...
			function getiev(){
				...
				$.browser.msie8 = $.browser.msie
						&& /msie 8\.0/i.test(userAgent);
				$.browser.msie7 = $.browser.msie
						&& /msie 7\.0/i.test(userAgent);
