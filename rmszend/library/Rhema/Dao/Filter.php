...
class Rhema_Dao_Filter{
	...
	protected $_limit = self::QUERY_LIMIT;
	protected $_page = 1;
	...
	public function __construct($limit = self::QUERY_LIMIT, $page = 1){
		if($limit){
			$this->_limit = $limit;
		}
	...
	public function setSearchCriteria(array $options = null, $columns = array()){
		if(empty($this->_conditions) and $options){
			...
			if(preg_match('/(menu)$/i', $this->_model)){ // and $doSearch == 'false'
				if($root_id){
