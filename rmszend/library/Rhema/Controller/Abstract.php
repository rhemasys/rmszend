...
abstract class Rhema_Controller_Abstract extends Zend_Controller_Action implements Rhema_Cache_Interface{
	...
	public function getBaseLink(){
		$request = $this->getRequest();
		$link = $this->_request->getScheme() . '://' . $this->_request->getHttpHost() . $this->_request->getBasePath() . '/';
		return $link;
	}
	...
	public function displayGrid($options = array(), $set = true){
		...
		$width = $this->_request->getParam('gridwidth');
 ...
 */
	...
	protected function _sendAjaxMessage($message, $title = null, $type = Rhema_Dto_UserMessageDto::TYPE_WARNING){
		$autoclose     = ($type == Rhema_Dto_UserMessageDto::TYPE_SUCCESS)? true : false;
