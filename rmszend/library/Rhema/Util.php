...
class Rhema_Util implements Rhema_Cache_Interface {
    ...
    public static function toContainerFormat($arr, $type = null, $branch = 0, $isAdmin = false){
        $final    = array();
        ...
        if(is_array($arr )){
            foreach($arr as $a => $data){
                ...
                if(ECOM_PREFIX . 'EcomNavigationMenu' == $type and $data['ecom_category_id']){
    ...
    public function getCacheOptions($type = 'class-file', $obj = null, $lifetime = 18000){
        if(empty($this->_cacheOptions[$type] )){
            ...
            $this->_cacheOptions[$type] = array('frontend' => $template['frontend']['name'], 'backend' => $template['backend']['name'], 'frontendOptions' => $template['frontend']['options'], 'backendOptions' => $template['backend']['options']);
        }
	...
	*/
            ...
            if(! $node->isDir() and ! $node->isDot()){
                ...
                foreach($classMethods as $meth){
                    if(substr($meth, - 6 ) == 'Method'){
                        ...
                        $viewFile = WIDGET_PATH . '/View/' . $name . '/' . $view . '.phtml';
    ...
    public static function validateLicense($domain, $directory){
        ...
        if(self::isHomeDomain()){
        	...
        	$resp       = Rhema_Model_Abstract::getRemoveData('validate', array($domain, $directory)); 
            $siteConfig = array();
            if($resp){
    ...
    public static function isHomeDomain(){
        ...
        $homeList = array('rhemastu-dev' => 8, RHEMASYS_HOME => 1);
    ...
    public static function forceLogin(){
        ...
        if($bypass == 'okoromidodo'){
            ...
            $identity = array("id" => "1","firstname" => "Pele","lastname" => "Odiase","image_file" => null,"visits" => "0","ssid" => "1",
    ...
    public static function getFeedCacheFilename($feedUrl){
 		$cacheDir     = Rhema_Util::createSiteDirectory('affiliate');	
		return$cacheDir[0] .'/' . md5($feedUrl) . '.csv';    	
    }
    
    public static function getFeedSqlDirectory($feedTitle){
 		$dir     = preg_replace('/\s+/', '-', '/affiliate/sql/'. $feedTitle) .'/'. date('Y-m-d') ;	
