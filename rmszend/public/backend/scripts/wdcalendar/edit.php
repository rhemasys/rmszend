    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function() {
           ...
           // var DATA_FEED_URL = "php/datafeed.php";
            ...
            jQuery("#timezone").val(new Date().getTimezoneOffset()/60 * -1);
            jQuery("#stparttime").dropdown({
                dropheight: 200,
                dropwidth:60,
                selectedchange: function() { },
                items: arrT
            });
            jQuery("#etparttime").dropdown({
                dropheight: 200,
                dropwidth:60,
                selectedchange: function() { },
                items: arrT
            });
            var check = jQuery("#IsAllDayEvent").click(function(e) {
                if (this.checked) {
                    jQuery("#stparttime").val("00:00").hide();
                    jQuery("#etparttime").val("00:00").hide();
                }
                else {
                    var d = new Date();
                    ...
                    if (p > 30) p = p - 30;
                    d = DateAdd("n", p, d);
                    jQuery("#stparttime").val(getHM(d)).show();
                    jQuery("#etparttime").val(getHM(DateAdd("h", 1, d))).show();
                }
            });
            if (check[0].checked) {
                jQuery("#stparttime").val("00:00").hide();
            ...
            jQuery("#Savebtn").click(function() { jQuery("#fmEdit").submit(); });
            jQuery("#Closebtn").click(function() { CloseModelWindow(); });
            jQuery("#Deletebtn").click(function() {
                 if (confirm("Are you sure to remove this event")) {  
                    var param = [{ "name": "calendarId", value: 8}];                
                    jQuery.post(DATA_FEED_URL + "?method=remove",
                        param,
                        function(data){
                              if (data.IsSuccess) {
                                    alert(data.Msg); 
           ...
           jQuery("#stpartdate,#etpartdate").datepicker({ picker: "<button class='calpick'></button>"});    
            var cv =jQuery("#colorvalue").val() ;
            if(cv=="")
            {
                cv="-1";
            }
            jQuery("#calendarcolor").colorselect({ title: "Color", index: cv, hiddenid: "colorvalue" });
            ...
            jQuery.validator.addMethod("safe", function(value, element) {
                return this.optional(element) || /^[^$\<\>]+$/.test(value);
            }, "$<> not allowed");
            jQuery("#fmEdit").validate({
                submitHandler: function(form) { jQuery("#fmEdit").ajaxSubmit(options); },
                errorElement: "div",
