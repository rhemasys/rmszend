...
class Rhema_Grid_Service{
	...
	public function generateGrid(){		
		...
		if($this->getType() == self::TYPE_JQGRID){
			...
			foreach($modelparams as $item){
				...
				}elseif(substr($item->name, 0, 6) == 'image_' or substr($item->name, - 5) == 'thumb'){
					$updateValue['format'] = array( 'image',  array('src' => '{{' . $item->name . '}}'));
	...
	public function getDaoFilter(){
		...
		if(isset($postVars[$limitIndex])){
			$limit = $postVars[$limitIndex];
	...
	public static function formatUpdateButton($id){
		$view = Zend_Layout::getMvcInstance()->getView();
		$url  = $view->url(array('task' => 'update', 'feedId' => $id), 'preview-mapping');
